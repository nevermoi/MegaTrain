...
@dataclass
class MegaTrainEngineConfig(EngineConfig):
    """Configuration for MegaTrain CPU-memory-centric single-GPU engine.

    MegaTrain stores parameters in CPU RAM and uses GPU as transient compute,
    enabling training of 100B+ models on a single GPU.

    Args:
        device: CUDA device index (default: 0)
        attn_implementation: HF attention implementation
        checkpoint_interval: Gradient checkpoint every N layers
        num_grad_slabs: Number of gradient slab buffers
        max_seq_len: Maximum sequence length for MegaTrain
    """
    ...
    # Override: MegaTrain always offloads
    param_offload: bool = True
    optimizer_offload: bool = True

    def __post_init__(self) -> None:
        assert self.strategy == "megatrain", f"strategy must be 'megatrain', got {self.strategy}"
